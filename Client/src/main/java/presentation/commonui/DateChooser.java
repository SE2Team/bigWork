...
public class DateChooser extends JPanel {
    ...
    public DateChooser(String format, JTextField jtf) {
    ...
    private void initLabel() {
        this.setSize(30, 30);
        this.setLayout(null);
        showDate = new JLabel(new ImageIcon("images/日历.jpg"));
    ...
    public String commit() {
        date_jtf.setText(sdf.format(select.getTime()));
        hidePanel();
...
//                this.setForeground(Color.green);
             ...
             * 判断当前月面板日期是否可以被选中
             */
            if ((day > select.get(Calendar.DAY_OF_MONTH) && month == now.get(Calendar.MONTH)
                    && year == now.get(Calendar.YEAR)) || month != select.get(Calendar.MONTH)
                    || year > now.get(Calendar.YEAR) || (year == now.get(Calendar.YEAR) && month > now.get(Calendar.MONTH))) {
                this.setEnabled(false);
                this.invalid = true;
            }
        }
